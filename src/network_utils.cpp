...
gateKeeper::gateKeeper()
    : sockfd(socket(AF_INET, SOCK_DGRAM, 0)), bound_to(-1)
{
    if (this->sockfd < 0) {
...
int gateKeeper::bindTo(uint32_t ipv4, uint16_t port) {
    sockaddr_in sv_info{};
    sv_info.sin_family = AF_INET;
    sv_info.sin_port = port;
    sv_info.sin_addr.s_addr = ipv4;

    int res = ::bind(this->sockfd, reinterpret_cast<const struct sockaddr*>(&sv_info), sizeof(sv_info));
    if (res < 0) {
...
int gateKeeper::send(uint32_t ipv4, uint16_t port, const char* data, uint32_t size) {
    sockaddr_in dst_info{};
    dst_info.sin_family = AF_INET;
    dst_info.sin_port = port;
    dst_info.sin_addr.s_addr = ipv4;

    int amt = sendto(this->sockfd, data, size, 0,
...
int gateKeeper::receive(uint32_t ipv4, uint16_t port, portMessage* res) {
